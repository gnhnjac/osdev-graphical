...
// loads mz executable into virtual memory and returns entry point address
PImageInfo load_mz_executable(pdirectory *pdir, char *path)
{
    ...
    printf("%d,%d,%d,%U,%U,%U,%U",dosHeader->e_maxalloc,dosHeader->e_cp,dosHeader->e_cparhdr*16-sizeof(IMAGE_DOS_HEADER),dosHeader->e_ss,dosHeader->e_sp,dosHeader->e_cs,dosHeader->e_ip);
